...
void exercise_1()
{
	...
	T_exercise1_state exercise1_state = MOTOR_STOPPED;
	...
	while (true)
	{
		...
		case MOTOR_RUNNING:
			...
			while (getMotorEncoder(motor1) <= 627) {
				...
				writeDebugStreamLine("While loop. MOTOR_RUNNING STATE.  Encoder = %d", getMotorEncoder(motor1));
				// If button2 is pushed stop motor after we are done rotating one full radian
				if (SensorValue(button2)) {
					// If button 2 pushed, transition to MOTOR_STOP state.
					motor_state = MOTOR_STOPPED;
					// Set flag to indicate that button 2 processed.
					button2_pushed = true;
					}else {
...
void exercise_3()
{
	...
	while (true)
	{
		...
		switch (motor_state) {
			...
			// Code for MOTOR_STOP state:
		case MOTOR_STOPPED:
			//writeDebugStreamLine("MOTOR_STOPPED STATE.\n");
			// Turn motor off.
			motor[motor1] = 0;
			// Reset the encoder
			resetMotorEncoder(motor1);
			if (button1_pushed) {;
				writeDebugStreamLine("MOTOR_STOPPED STATE. Button1 pushed. Changing state to MOTOR_FORWARDS.");
				...
				writeDebugStreamLine("MOTOR_STOPPED STATE. Button2 pushed. Changing state to MOTOR_BACKWARDS.");
			...
			break;
		case MOTOR_FORWARDS:
			writeDebugStreamLine("MOTOR_FORWARDS STATE.");
			resetMotorEncoder(motor1);
			// Turn motor on.
			writeDebugStreamLine("MOTOR_FORWARDS STATE. Turning on motor.");
			motor[motor1] = 50;
			while (getMotorEncoder(motor1) <= 3000) {
				writeDebugStreamLine("While loop. MOTOR_FORWARDS STATE.  Encoder = %d", getMotorEncoder(motor1));
				if(SensorValue(button2)){
					writeDebugStreamLine("#####MOTOR_FORWARDS STATE. Button2 pushed. Changing state to MOTOR_BACKWARDS.#####");
					motor_state = MOTOR_BACKWARDS;
					// Set flag to indicate that button 2 processed.
					button2_pushed = true;
				}
					...
					writeDebugStreamLine("#####MOTOR_FORWARDS STATE. Button1 pushed. Do nothing but set button1_pushed to true.#####");
					button1_pushed = false;
				}
				else{
					motor_state = MOTOR_STOPPED;
		...
		case MOTOR_BACKWARDS:
			writeDebugStreamLine("MOTOR_BACKWARDS STATE.");
			resetMotorEncoder(motor1);
			// Turn motor on.
			motor[motor1] = -50;
			while (getMotorEncoder(motor1) >= -3000) {
				writeDebugStreamLine("While loop. MOTOR_BACKWARDS STATE. Encoder = %d", getMotorEncoder(motor1));
				if (SensorValue(button1)) {
					writeDebugStreamLine("#####MOTOR_BACKWARDS STATE. Button1 pushed. Changing state to MOTOR_FORWARDS.#####");
					motor_state = MOTOR_FORWARDS;
					// Set flag to indicate that button 2 processed.
					button1_pushed = true;
				}
				else if (SensorValue(button2)) {
					writeDebugStreamLine("#####MOTOR_BACKWARDS STATE. Button2 pushed. Do nothing but set button2_pushed flag.#####");
